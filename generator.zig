...
pub fn generate(buffer : []u8, out_buffer : *Array(u8)) !void {
    ...
    for(buffer) |char| {
        ...
        switch(char) {
            '>' => try out_buffer.appendSlice("state.push_data_pointer();"),
            '<' => try out_buffer.appendSlice("state.pop_data_pointer();"),
            '+' => try out_buffer.appendSlice("state.increment_current_data();"),
            '-' => try out_buffer.appendSlice("state.decrement_current_data();"),
